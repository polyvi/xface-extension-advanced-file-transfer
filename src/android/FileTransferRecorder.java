...
public class FileTransferRecorder {
     ...
     *
     * @param info
     *            要存储的下载信息，包括文件的地址，文件的大小和已下载的大小
     */
    public synchronized void saveDownloadInfo(FileDownloadInfo info) {
        if (null != mDocument) {
            Element downloadInfoElement = (Element) mDocument
            ...
            downloadElement.setAttribute(CONFIG_FILE_TAG_COMPLETE_SIZE,
                    String.valueOf(info.getCompleteSize()));
            downloadElement.setAttribute(CONFIG_FILE_TAG_TOTAL_SIZE,
                    String.valueOf(info.getTotalSize()));
     ...
     *            要获取的路径
     */
    public synchronized FileDownloadInfo getDownloadInfo(String url) {
        FileDownloadInfo info = null;
        if (null != mDocument) {
            Element downloadElement = mDocument.getElementById(url);
            if (downloadElement != null) {
                info = new FileDownloadInfo(Integer.parseInt(downloadElement
                        .getAttribute(CONFIG_FILE_TAG_TOTAL_SIZE)),
                        Integer.parseInt(downloadElement
                                .getAttribute(CONFIG_FILE_TAG_COMPLETE_SIZE)),
                        url);
            }
        }
        return info;
     ...
     *            要上传的文件地址
     */
    public synchronized void saveUploadInfo(String sourceid, String filePath,
            String totalSize) {
        if (null != mDocument) {
